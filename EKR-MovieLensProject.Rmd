...
Two of these models already seem to show adequate performance out of the box, with $RMSE_{pop} =$ 
`r benchmark_result9["POPULAR",]$RMSE` in $t_{pop} =$ `r benchmark_result9["POPULAR",]$time` s
and $RMSE_{libmf} =$ `r benchmark_result9["LIBMF",]$RMSE` in $t_{libmf} =$ `r benchmark_result9["LIBMF",]$time` s. Before tuning, SVD seems to lag behind, both in RMSE (`r benchmark_result9["SVD",]$RMSE`) and time (`r benchmark_result9["SVD",]$time` s) performance.

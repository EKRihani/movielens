...
```  
Quite surprisingly, our results show that the Z-score normalization, albeit more refined, produces less accurate predictions than the center normalization.
...
plot(plot_fitting5 + 
   ...
   theme_bw())
```  
Raising the k parameter above the default value improves sensibly the accuracy of the prediction. However, this has a cost in computing time. Getting RMSE below the optimal value (RMSE <= 0.865) requires to set k >= 50 (RMSE = `r results_fitting[which(results_fitting$rmse <= 0.865 & results_fitting$model == "SVD" & results_fitting$parameter == "k")[1],]$rmse` in `r results_fitting[which(results_fitting$rmse <= 0.865 & results_fitting$model == "SVD" & results_fitting$parameter == "k")[1],]$running_time` s).

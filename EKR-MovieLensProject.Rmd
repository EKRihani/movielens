...
```{r echo=FALSE}
#par(mar = c(4, 4, .2, .1))
plot(plot_time_rmse2, pch = 19)
plot(plot_time_rmse3, pch = 17)
```
Bigger datasets seem to confirm the respective places of the SVD, UBCF, LIBMF and Popular methods.  

```{r echo=FALSE}
#par(mar = c(4, 4, .2, .1))
plot(plot_time_size1, pch = 19)
plot(plot_time_size2, pch = 17)
plot(plot_time_size3, pch = 17)
```  
The computing time vs dataset size scatterplot seems to show a lower computing time for SVD, Popular and LIMBF models, while the UBCF model appears to be considerably slower. Furthermore, the computing time of this model seems to exhibit a quadratic behaviour, whereas the others seem to be linear.  
...
plot(plot_rmse_size)
```  
According to the RMSE vs size scatterplot, our 3 selected models seem to show stability for $size \ge 0.2$.  
Two of these models already show adequate performance out of the box, with $RMSE_{pop} =$ 
<!-- [INSERER BMR9-RMSE ICI] --> in $t_{pop} =$ <!-- [INSERER BMR9-TEMPS ICI] --> s
and $RMSE_{limbf} =$ <!-- [INSERER BMR9-RMSE ICI] --> in $_{limbf} =$ <!-- [INSERER BMR9-TEMPS ICI] --> s. Before tuning, SVD seems to lag behind, both in RMSE (<!-- [INSERER BMR9-RMSE ICI] -->) and time (<!-- [INSERER BMR9-RMSE ICI] --> s) performance.

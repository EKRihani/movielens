...
\subsection{Comparing the performance of the different models}
\paragraph*{}
...
```
\paragraph*{}
The *dim* parameter has a major effect on accuracy and computing time. Higher values yield a better accuracy, with RMSE = `r results_fitting[results_fitting$model == "LIBMF" & results_fitting$parameter == "dim" & results_fitting$value == "400",]$rmse`, at the cosr of a higher computing time (t = `r results_fitting[results_fitting$model == "LIBMF" & results_fitting$parameter == "dim" & results_fitting$value == "400",]$time`).
\paragraph*{}
...
\paragraph*{}
We can for example choose to put a 2:1 weight on RMSE vs time (*n* = 2), to give priority to RMSE -- which is the primary goal of this project -- while also taking computing time into account.
```{r out.width = "90%", echo = FALSE}
plot(plot_fitting1b + theme_bw())
```  
For *n* = 2, an optimum is reached for $dim \approx 400$.
...
\paragraph*{}
We manage to obtain RMSE = `r final_rmse` in `r final_time`

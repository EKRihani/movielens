...
```
\paragraph*{}
The *dim* parameter has a major effect on accuracy and computing time. Higher values ($dim = 1000$) yield a better accuracy, with RMSE = `r results_fitting[results_fitting$model == "LIBMF" & results_fitting$parameter == "dim" & results_fitting$value == "1000",]$rmse`, at the cost of a higher computing time (t = `r results_fitting[results_fitting$model == "LIBMF" & results_fitting$parameter == "dim" & results_fitting$value == "1000",]$time` s).
\paragraph*{}
...
```
\paragraph*{}
Using this 2:1 RMSE/time composite factor (*n* = 2), an optimum seems to be reached for a dimension value of $dim \approx 400$.
\paragraph*{}
...
\paragraph*{}
The accuracy seems to considerably improve for lower values of *costp_l2* and *costq_l2*. However, for values below 0.01  (default value), the accuracy gains seem to be quite marginal.
\paragraph*{}
Apart from some outliers, most of the *costp_l2* and *costq_l2* values seem to have little effect on computing time, with most results distributed in a  `r round(sd(results_fitting[results_fitting$model == "LIBMF" & results_fitting$parameter %in% c("costp_l2","costq_l2"),]$time) / sqrt(nrow(results_fitting[results_fitting$model == "LIBMF" & results_fitting$parameter %in% c("costp_l2","costq_l2"),]))*2*qnorm(.975),2)`s time span (width of the 95% IC).
...
```
\paragraph*{}
The number of threads (*nthread*) seem to have some meaningful impact in computing time. The RMSE varies very slightly from one *ntrehad* setting to another, but the difference is very small ($\Delta_{RMSE} \approx$ `r round(max(results_fitting[results_fitting$model == "LIBMF" & results_fitting$parameter == "nthread",]$rmse) - min(results_fitting[results_fitting$model == "LIBMF" & results_fitting$parameter == "nthread",]$rmse),3)`) This is an interesting result that shows that the LIMBF multithreading works out of the box, without having to install additional libraries or set the parallelization manually.
\paragraph*{}
However, the improvement in computing time can seem disappointing, since the impact is pretty minor, especially for $nthreads \geq 4$.
\paragraph*{}

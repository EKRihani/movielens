...
The metric used in this study to evaluate the accuracy of the prediction is the root-mean-squared error (RMSE), given by : $$\sqrt{ \frac{1}{N} \sum_{u,i}\left(\hat{y}_{i} -  y_{i}\right)^{2}}$$ With N the number of ratings, $\hat{y}_{i}$ the rating that is predicted by the model built with the training set and $y_{i}$ the actual rating of the testing set. The lower the RMSE, the more accurate the model.  
...
The movie-effect model ca thus be improved to $\mathbf{Y}_{u,m} = \beta + \alpha_{m} + \alpha_{u} + \varepsilon_{u,m}$, by taking into account the average "generosity" of each user ($\alpha_{u}$).  
